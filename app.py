...
with st.sidebar:
    ...
    section = st.radio("Sección",
        ["Resumen","Consulta","Agenda","Gantt","Heatmap","Conflictos",
         "Disponibilidad","Delegaciones","Calidad","Diferencias","Recomendador","Diagnóstico","Acerca de"], index=0)
...
# Precalcular campos y tipos (optimiza memoria/velocidad)
...
def _is_weekday(d: Optional[date]) -> bool: return (d is not None) and (0 <= d.weekday() <= 4)
def _only_sep_oct_weekdays(d: Optional[date]) -> bool: return _is_weekday(d) and (d.month in (9,10))
...
# ========= Delegaciones desde archivo (VECTORIZADO) =========
...
# ========= Omnibox =========
c_omni = st.text_input("🔎 Búsqueda rápida (persona / mesa / aula)",
...
if section == "Resumen":
    ...
    with st.expander("💾 Vistas guardadas"):
        if st.button("Guardar vista actual"):
            payload = {"rng": (fmin.isoformat(), fmax.isoformat()),
                       ...
                       "sdel": solo_deleg, "q": st.query_params.get("q","")}
    ...
    if solo_deleg: mask &= idx["__delegado_por_archivo"]
    if term:
    ...
    def _actors_for_event_batch(df_events: pd.DataFrame) -> pd.DataFrame:
        ...
        ini_d  = merged["__ini_m"].to_numpy(); fin_d = merged["__fin_m"].to_numpy()
    ...
    st.markdown("Publicación: 13/09/2025 — INIMAGINABLE Optimizado (delegaciones vectorizadas, cachés y filtros rápidos)")
